...
(define-map daily {who: principal, day: uint} {planted: uint, verified: uint, carbon: uint})
(define-map weekly {who: principal, week: uint} {planted: uint, verified: uint, carbon: uint})
(define-map monthly {who: principal, month: uint} {planted: uint, verified: uint, carbon: uint})

;; Leaderboard lists
(define-data-var all-time-planted (list 10 {who: principal, score: uint}) (list))
(define-data-var all-time-verified (list 10 {who: principal, score: uint}) (list))
(define-data-var all-time-carbon (list 10 {who: principal, score: uint}) (list))

(define-map daily-planted {day: uint} (list 10 {who: principal, score: uint}))
(define-map daily-verified {day: uint} (list 10 {who: principal, score: uint}))
(define-map daily-carbon {day: uint} (list 10 {who: principal, score: uint}))

(define-map weekly-planted {week: uint} (list 10 {who: principal, score: uint}))
(define-map weekly-verified {week: uint} (list 10 {who: principal, score: uint}))
(define-map weekly-carbon {week: uint} (list 10 {who: principal, score: uint}))

(define-map monthly-planted {month: uint} (list 10 {who: principal, score: uint}))
(define-map monthly-verified {month: uint} (list 10 {who: principal, score: uint}))
(define-map monthly-carbon {month: uint} (list 10 {who: principal, score: uint}))
...
(define-private (get-daily-helper (who principal) (day uint))
  (default-to {planted: u0, verified: u0, carbon: u0}
    (map-get? daily {who: who, day: day})
  )
)

(define-private (get-weekly-helper (who principal) (week uint))
  (default-to {planted: u0, verified: u0, carbon: u0}
    (map-get? weekly {who: who, week: week})
  )
)

(define-private (get-monthly-helper (who principal) (month uint))
  (default-to {planted: u0, verified: u0, carbon: u0}
    (map-get? monthly {who: who, month: month})
  )
)
...
(define-public (inc-verified (who principal) (by uint))
  (begin
    (asserts! (> by u0) err-zero)
    (let ((day (current-day))
          (week (current-week))
      ...
      (let ((new-all-time-verified (unwrap! (safe-add (get verified current-all-time) by) err-overflow)))
        ...
        (var-set all-time-verified 
          (update-leaderboard (var-get all-time-verified) who new-all-time-verified))
      )
      ...
      (let ((new-daily-verified (unwrap! (safe-add (get verified current-daily) by) err-overflow)))
        ...
        (map-set daily-verified {day: day}
          (update-leaderboard (default-to (list) (map-get? daily-verified {day: day})) who new-daily-verified))
      )
      ...
      (let ((new-weekly-verified (unwrap! (safe-add (get verified current-weekly) by) err-overflow)))
        ...
        (map-set weekly-verified {week: week}
          (update-leaderboard (default-to (list) (map-get? weekly-verified {week: week})) who new-weekly-verified))
      )
      ...
      (let ((new-monthly-verified (unwrap! (safe-add (get verified current-monthly) by) err-overflow)))
        ...
        (map-set monthly-verified {month: month}
          (update-leaderboard (default-to (list) (map-get? monthly-verified {month: month})) who new-monthly-verified))
      )
...
(define-public (inc-carbon (who principal) (by uint))
  (begin
    (asserts! (> by u0) err-zero)
    (let ((day (current-day))
          (week (current-week))
      ...
      (let ((new-daily-carbon (unwrap! (safe-add (get carbon current-daily) by) err-overflow)))
        ...
        (map-set daily-carbon {day: day}
          (update-leaderboard (default-to (list) (map-get? daily-carbon {day: day})) who new-daily-carbon))
      )
      ...
      (let ((new-weekly-carbon (unwrap! (safe-add (get carbon current-weekly) by) err-overflow)))
        ...
        (map-set weekly-carbon {week: week}
          (update-leaderboard (default-to (list) (map-get? weekly-carbon {week: week})) who new-weekly-carbon))
      )
      ...
      (let ((new-monthly-carbon (unwrap! (safe-add (get carbon current-monthly) by) err-overflow)))
        ...
        (map-set monthly-carbon {month: month}
          (update-leaderboard (default-to (list) (map-get? monthly-carbon {month: month})) who new-monthly-carbon))
      )
...
(define-read-only (get-daily (who principal) (day uint))
  (get-daily-helper who day)
)

(define-read-only (get-weekly (who principal) (week uint))
  (get-weekly-helper who week)
)

(define-read-only (get-monthly (who principal) (month uint))
